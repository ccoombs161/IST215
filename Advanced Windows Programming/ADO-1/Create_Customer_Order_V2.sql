...
IF EXISTS(SELECT name FROM sys.databases WHERE name = 'CUSTOMER_ORDER_V2')
     DROP DATABASE CUSTOMER_ORDER_V2
GO

CREATE DATABASE CUSTOMER_ORDER_V2;

GO

USE CUSTOMER_ORDER_V2;
GO
...
CREATE TABLE [ORDER](
		...
		CONSTRAINT 		CUSTOMER_ORDER_V2_FK	FOREIGN KEY (CustomerID)
...
INSERT INTO [ORDER] VALUES(
		'VK001', 'DVD Video', 'Kitchen Remodeling Basics', 1, 19.99, 4);
INSERT INTO [ORDER] VALUES(
		'VK002', 'DVD Video', 'Advanced Kitchen Remodeling', 1, 19.99, 5);
INSERT INTO [ORDER] VALUES(
		'VK003', 'DVD Video', 'Kitchen Remodeling Dallas Style', 1, 19.99, 2);
INSERT INTO [ORDER] VALUES(
		'VK004', 'DVD Video', 'Heather Sweeny Seminar Live in Dallas on 25-OCT-07', 2, 19.99, 2 );
INSERT INTO [ORDER] VALUES(
		'VB001', 'Video Companion', 'Kitchen Remodeling Basics Video Companion', 2, 20.00, 8);
INSERT INTO [ORDER] VALUES(
		'VB002', 'Video Companion', 'Advanced Kitchen Remodeling Video Companion', 2, 20.00, 2);
INSERT INTO [ORDER] VALUES(
		'VB003', 'Video Companion', 'Kitchen Remodeling Dallas Style Video Companion', 2, 20.00, 10);
INSERT INTO [ORDER] VALUES(
		'BK001', 'Book', 'Kitchen Remodeling Basics For Everyone', 122, 20.00, NULL );
INSERT INTO [ORDER] VALUES(
		'BK002', 'Book', 'Advanced Kitchen Remodeling For Everyone', 12, 20.00, NULL );
INSERT INTO [ORDER] VALUES(
		'BK003', 'Book', 'How to Program in C#', 2, 20.00, null);
INSERT INTO [ORDER] VALUES(
		'BK004', 'Book', 'Datebase Design and Implementation', 3, 20.00, null);
...
SELECT * FROM [ORDER];
GO
-- DISPLAY All Customers and Orders, including Customers who do not have an Order.
-- using a LEFT join statement.
SELECT  C.CustomerID, O.OrderNum, C.CustomerName,  O.ProductName, O.ProductType
FROM CUSTOMER AS C LEFT JOIN [ORDER] AS O ON C.CustomerID = O.CustomerID

-- LEFT OUTER JOIN AND LEFT JOIN ARE SAME
SELECT  C.CustomerID, O.OrderNum, C.CustomerName,  O.ProductName, O.ProductType
FROM CUSTOMER AS C LEFT OUTER JOIN [ORDER] AS O ON C.CustomerID = O.CustomerID
--------------------------------------------------------------------------------
-- RIGHT JOIN -------------------------------------------------------------
-- DISPLAY All Orders, even if no customer is assigned to that order.
-- Use a RIGHT join statement
SELECT  O.OrderNum, O.ProductName, O.ProductType, O.CustomerID 
...
SELECT  C.CustomerID, O.OrderNum, C.CustomerName,  O.ProductName, O.ProductType, O.Quantity, O.UnitPrice, O.TotalPrice
FROM CUSTOMER AS C JOIN [ORDER] AS O ON C.CustomerID = O.CustomerID;
