    ...
    public partial class PersonGUI : Form
    {
        ...
        private void btnConnect_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                conn.ConnectionString = "Data Source=localhost;Initial Catalog=AddressBook;Integrated Security=True";
                //conn.ConnectionString = "server=.; uid=sa; pwd=nhti; database=AddressBook;";

                                                // 4 Pass the connection information to sql commands
                cmdObject.Connection = conn;
                
                //set the sql commands command to select all from the [tablename]
                ...
                //set the datapaters command = cmdObject. It will run the sql select statement to get all the data
                da.SelectCommand = cmdObject;
                
                //adapter object executes the select statement to fill the dataset with its data                
                da.Fill(ds);                
                ...
                txtEmail.Text = personList[0].Email.ToString();
                txtPhone.Text = personList[0].Phone.ToString();

                //display the connected label
                lblConnectStatus.Visible = true;
                //set the connect labels text
                lblConnectStatus.Text = "Connected Successfully";
            }
            //catch
            catch (Exception ex)
            {
